"""
Jazidas Cache Helpers
======================

Specialized cache patterns for the Jazidas MCP Server.
Built on top of RedisCache (mcp_servers.common.redis_cache).

Patterns:
    - Paginated search results (buscar_fornecedores, buscar_jazidas)
    - Process detail cache (detalhes_processo)
...
The pagination strategy:
    ...
    3. TTL: 1h for searches, 24h for process details
"""
...
logger = logging.getLogger("mcp.jazidas.cache")


class JazidasCache:
    """
    Cache helpers for Jazidas-specific data patterns.

    Usage:
        cache = JazidasCache(redis_cache)

        # Store full search results
        cache_id = await cache.store_search("fornec", params, all_results)
    ...
    # Cache key prefixes
    PREFIX_FORNECEDORES = "jazidas:fornec"
    PREFIX_JAZIDAS = "jazidas:search"
    PREFIX_PROCESSO = "jazidas:proc"
    ...
    async def store_search(
        ...
        Args:
            prefix: Cache key prefix (e.g., PREFIX_FORNECEDORES)
    ...
    # ==================================================================
    # PROCESS DETAILS (single document cache)
    # ==================================================================

    def _processo_key(self, ds_processo: str) -> str:
        """Generate cache key for a specific process."""
        # Normalize: "832.145/2018" → "832145_2018"
        normalized = ds_processo.replace(".", "").replace("/", "_").strip()
        return f"{self.PREFIX_PROCESSO}:{normalized}"

    async def get_processo(self, ds_processo: str) -> dict | None:
        """
        Retrieve cached process details.

        Args:
            ds_processo: Process code (e.g., "832.145/2018")

        Returns:
            Process data dict or None if not cached
        """
        key = self._processo_key(ds_processo)
    ...
    async def store_processo(self, ds_processo: str, data: dict) -> bool:
        """
        Cache process details.

        Args:
            ds_processo: Process code
            data: Complete process data dict
        ...
        ttl = mcp_settings.cache_substancia_ttl  # 24h (reuses the 24h TTL)
        stored = await self.redis.set(key, payload, ttl=ttl)
        if stored:
            logger.debug(f"Cached process {ds_processo} → {key}")
        return stored
    ...
    async def store_mapa(self, cache_id: str, mapa: dict, ttl: int | None = None) -> None:
        """
        Cache mapa data (pontos + optional geometrias) alongside search results.

        Uses key = {cache_id}:mapa with the same TTL as the search.
        """
        ttl = ttl or mcp_settings.cache_search_ttl
    ...
    async def get_mapa(self, cache_id: str) -> dict | None:
        """Retrieve cached mapa data for a search, or None if not found."""
    ...
    async def invalidate_processo(self, ds_processo: str) -> bool:
        """Invalidate a specific cached process."""
        key = self._processo_key(ds_processo)
        return await self.redis.delete(key)
    ...
    async def store_geometry(
        ...
        """Store GeoJSON geometry collections alongside a cached search."""
        key = f"{cache_id}:geo"
        ttl = ttl or mcp_settings.cache_search_ttl
        payload = json.dumps(geo_data, default=str, ensure_ascii=False)
        return await self.redis.set(key, payload, ttl=ttl)

    async def get_geometry(self, cache_id: str) -> dict | None:
        """Retrieve GeoJSON geometry collections for a cached search."""
        key = f"{cache_id}:geo"
        raw = await self.redis.get(key)
        if not raw:
            return None
        try:
            ...
            logger.warning(f"Invalid geometry JSON in cache key {key}")
            return None
